...
1. **Session close (steps 1-6)** — walk the 5 mandatory memory files plus open-questions (conditional). `/compact` no longer waits on this; `--check-session-close` is the actual go/no-go signal for calling the session closed.
...
- **Close mode**: walks the checklist (session-state, project hot.md, root hot.md, session-log, open-questions(변경 시), log.md) plus a lint step, then writes via `/hypo:crystallize` in `--apply-session-close --payload=<path> --session-id=<id>` mode, which runs the lint gate automatically, **scoped to the files it writes** (debt elsewhere is a non-blocking notice). **`--session-id` is required.** Before writing a byte, the apply resolves that session's transcript and refuses the whole close, exit 1 and nothing on disk, unless the **user** asked for it (`session-id-required` / `transcript-unresolved` / `no-user-close-signal`). A refusal is the answer, not an obstacle: ask the user, and re-run only if they say yes. `--check-session-close` is a read-only dry-run of the **full** PreCompact gate: close files + scoped lint + design-history + feedback projection, sharing one function (`precompactGateStatus`) with the `hypo-personal-check.mjs` PreCompact hook. That hook itself no longer blocks `/compact`; it only turns a red result into a `systemMessage`. So a green check here is not "the gate that will stop `/compact`" but the same signal the hook would have surfaced, checked ahead of time: no close-file gap, lint blocker, stale design-history, or feedback-projection issue needs a human fix, which is the practical bar for declaring the session closed (pass `--transcript-path` to widen the lint scope to this session's edited files exactly as the interactive hook does). It is not a hard guarantee even at that lower bar: the live PreCompact run can still differ on a context-≥70% prompt, `HYPO_SKIP_GATE`, or a transcript-scoped lint error the check did not see, and none of that differs `/compact` itself, which proceeds either way.
...
This runs the **full** set of checks the `hypo-personal-check.mjs` PreCompact
hook itself runs, via the shared `precompactGateStatus`: close files
(`missing` / `stale`) plus lint blockers, stale design-history, and feedback
projection over-cap/conflict. Fix every `✗` it reports and re-run until it
prints **"Compact-ready"**, that is the signal the session is closed. A
close-files-only pass is not enough to earn that signal; the other checks
still gate it here, even though the live `/compact` no longer blocks on any
of them (that hook now only turns a red result into a `systemMessage`).
