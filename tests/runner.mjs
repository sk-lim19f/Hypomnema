...
suite('check-readme-version.mjs (IMPR-14)');

// Minimal fixture: a package.json plus both READMEs. The version string is
// embedded in prose so the boundary matcher is exercised the way the real
// rolling narrative embeds it.
function buildReadmeFixture(dir, { version, enText, koText }) {
  writeFileSync(join(dir, 'package.json'), JSON.stringify({ name: 'hypomnema', version }, null, 2));
  writeFileSync(join(dir, 'README.md'), enText);
  writeFileSync(join(dir, 'README.ko.md'), koText);
}

test('both READMEs mention the version → exit 0', () => {
  withTmpDir((dir) => {
    buildReadmeFixture(dir, {
      version: '1.4.0',
      enText: '# Hypomnema\n\nv1.4.0 (current release) ships the ship skill.\n',
      koText: '# Hypomnema\n\nv1.4.0(현재 릴리스)는 ship 스킬을 출시합니다.\n',
    });
    const r = run('check-readme-version.mjs', ['--root', dir]);
    assert.equal(r.status, 0, `expected 0: ${r.stdout}\n${r.stderr}`);
  });
});

test('version missing from one README → exit 1', () => {
  withTmpDir((dir) => {
    buildReadmeFixture(dir, {
      version: '1.4.0',
      enText: '# Hypomnema\n\nv1.4.0 shipped.\n',
    ...
    const r = run('check-readme-version.mjs', ['--root', dir]);
    assert.equal(r.status, 1, `expected 1 when ko README lacks the version: ${r.stdout}`);
    assert.ok(/README\.ko\.md/.test(r.stderr + r.stdout), 'should name the missing file');
  });
});

test('version missing from both READMEs → exit 1', () => {
  withTmpDir((dir) => {
    buildReadmeFixture(dir, {
      version: '1.4.0',
      enText: '# Hypomnema\n\nno version here.\n',
    ...
    const r = run('check-readme-version.mjs', ['--root', dir]);
    assert.equal(r.status, 1, 'expected 1 when neither README has the version');
  });
});
...
test('--version overrides package.json', () => {
  withTmpDir((dir) => {
    buildReadmeFixture(dir, {
      version: '1.4.0', // package.json says 1.4.0
      enText: '# Hypomnema\n\nv2.0.0 preview notes.\n',
      koText: '# Hypomnema\n\nv2.0.0 미리보기.\n',
    });
    // Assert against 2.0.0 (not package.json's 1.4.0) → both READMEs have it.
    assert.equal(run('check-readme-version.mjs', ['--root', dir, '--version', '2.0.0']).status, 0);
    // And the package.json version (1.4.0) is absent → default check fails.
    assert.equal(run('check-readme-version.mjs', ['--root', dir]).status, 1);
  });
});

test('a longer version is not a false match (1.3.40 ≠ 1.3.4)', () => {
  withTmpDir((dir) => {
    buildReadmeFixture(dir, {
      version: '1.3.4',
      // Only "1.3.40" appears — the boundary matcher must NOT accept it as 1.3.4.
      enText: '# Hypomnema\n\nsee the old v1.3.40 line.\n',
      koText: '# Hypomnema\n\n예전 v1.3.40 줄 참조.\n',
    });
    const r = run('check-readme-version.mjs', ['--root', dir]);
    assert.equal(r.status, 1, 'a 1.3.40 substring must not satisfy a 1.3.4 check');
  });
});

test('version at a sentence boundary still matches (trailing period)', () => {
  withTmpDir((dir) => {
    buildReadmeFixture(dir, {
      version: '1.3.4',
      enText: '# Hypomnema\n\nThis is shipped as v1.3.4.\n',
      koText: '# Hypomnema\n\nv1.3.4로 출시되었습니다.\n',
    });
    const r = run('check-readme-version.mjs', ['--root', dir]);
    assert.equal(r.status, 0, `trailing period must not break the match: ${r.stdout}\n${r.stderr}`);
  });
});

test('a stable check does NOT false-pass on a prerelease string', () => {
  withTmpDir((dir) => {
    // Only the RC string appears — a check for stable 1.3.4 must fail, otherwise
    // shipping 1.3.4 after 1.3.4-rc.1 would skip the README reconcile entirely.
    buildReadmeFixture(dir, {
      version: '1.3.4',
      enText: '# Hypomnema\n\nlatest preview v1.3.4-rc.1.\n',
      koText: '# Hypomnema\n\n최신 프리뷰 v1.3.4-rc.1.\n',
    });
    const r = run('check-readme-version.mjs', ['--root', dir]);
    assert.equal(r.status, 1, '1.3.4 must not match inside 1.3.4-rc.1');
  });
});

test('a prerelease version matches its exact string but not a longer one', () => {
  withTmpDir((dir) => {
    buildReadmeFixture(dir, {
      version: '1.3.0-rc.1',
      enText: '# Hypomnema\n\nshipping v1.3.0-rc.1 today.\n',
      koText: '# Hypomnema\n\n오늘 v1.3.0-rc.1 출시.\n',
    });
    assert.equal(
      run('check-readme-version.mjs', ['--root', dir]).status,
      0,
      'exact rc string present',
    );
    // A README that only has rc.10 must NOT satisfy an rc.1 check.
    writeFileSync(join(dir, 'README.ko.md'), '# Hypomnema\n\nv1.3.0-rc.10 only.\n');
    assert.equal(
      run('check-readme-version.mjs', ['--root', dir]).status,
      1,
      'rc.1 must not match inside rc.10',
    );
  });
});

test('an unreadable README → exit 1 (no silent pass)', () => {
  withTmpDir((dir) => {
    buildReadmeFixture(dir, {
      version: '1.4.0',
      enText: '# Hypomnema\n\nv1.4.0.\n',
    ...
    rmSync(join(dir, 'README.ko.md'));
    const r = run('check-readme-version.mjs', ['--root', dir]);
    assert.equal(r.status, 1, 'a missing README file must fail');
  });
});

test('--json reports per-file presence', () => {
  withTmpDir((dir) => {
    buildReadmeFixture(dir, {
      version: '1.4.0',
      enText: 'v1.4.0\n',
    ...
    const r = run('check-readme-version.mjs', ['--root', dir, '--json']);
    const out = JSON.parse(r.stdout);
    assert.equal(out.ok, false);
    assert.equal(out.version, '1.4.0');
    const ko = out.results.find((x) => x.file === 'README.ko.md');
    assert.equal(ko.present, false, 'ko README should report present:false');
  });
});
