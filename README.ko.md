...
위키 데이터는 로컬 파일로 저장됩니다. Git remote를 설정하면 Stop 훅이 자동으로 commit 및 push합니다 — 서드파티 동기화 서비스는 관여하지 않습니다. Claude API를 사용하는 커맨드(`/hypo:verify`, `/hypo:lint --llm`)는 페이지 내용을 Anthropic에 전송합니다. 그 외 모든 훅은 완전히 오프라인으로 동작합니다.
