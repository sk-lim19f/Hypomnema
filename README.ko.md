...
**Step 2: Claude Code 안에서 초기화**

```
/hypo:init
```

이 한 번의 명령이 다음을 수행합니다:

- `~/hypomnema/`(또는 원하는 경로)에 위키 생성
- `pages/`, `projects/`, `sources/`, `journal/{daily,weekly,monthly}/` 및 루트 마커 스캐폴딩
- 10개의 라이프사이클 훅 설치 + `~/.claude/settings.json` 병합
- 로컬 git 저장소 초기화 + 첫 커밋 + (remote 있으면) push
...
이게 전부입니다. 나머지는 훅이 처리합니다 — 자동 staging, 자동 commit/push, 세션 상태 주입, 룩업 신호.
...
| 명령어 | 하는 일 | 언제 쓰나 |
|---|---|---|
| `/hypo:ingest` | 원본을 `sources/`에 보관, 구조화된 페이지를 `pages/`에 합성 | 보관할 가치가 있는 글을 읽었을 때 |
...
1. `HYPO_DIR` 환경변수 (명시적 오버라이드)
2. `hypo-config.md` 스캔 — 현재 디렉터리부터 `$HOME`까지 상향
3. 기본값: `~/hypomnema`
