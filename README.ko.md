...
현재 릴리스는 v1.5.0입니다. 머신 간 안정성과 일상의 마찰을 줄이는 쪽으로 초점을 옮겼습니다. git으로 동기화하는 볼트에서 `cwd`-first resume가 올바른 프로젝트를 찾고, 코드 레포에서 작업하는 세션은 매번 위키를 다시 찾는 대신 위치를 안내받으며, 세션 마무리는 무관한 lint 부채를 다시 나열하거나 경고 수백 개를 컨텍스트에 쏟지 않습니다. 그 앞 v1.4.x는 세션 마무리 경로를 견고하게 다듬었습니다.
