...
> **현재 상태 vs. v2 비전.** v1.2.0(현재)은 트리거 모델을 정직하게 밝힙니다: 대부분의 위키 동작 — 인제스트, 쿼리, 세션 클로즈 — 은 여전히 **명시적 `/hypo:*` 커맨드**로 시작되지만, 자율 동작 surface가 점차 확장되고 있습니다. v2 thesis는 *완전 자율* — Claude가 요청 없이 위키를 읽고·쓰고·합성하는 상태입니다. **v1.1.0**은 그 램프의 첫 단계로 **observability 점수**(ingest/query/session-close/citation 비율)를 도입했고, **v1.2.0**은 그 위에 자율 동작 4개를 더 얹었습니다:
> - **feedback을 단일 진실원천(SoT)으로** — `pages/feedback/`이 행동 교정의 유일한 source-of-truth가 되고, 위키가 단방향으로 `MEMORY.md`와 `~/.claude/CLAUDE.md`의 `<learned_behaviors>` 블록을 자동 생성합니다. 한 곳만 편집하면 두 곳이 알아서 따라옵니다 (설계 의도와 근거는 위키 안의 `projects/hypomnema/decisions/0031-*.md` 참조).
> - **extensions 동봉 자동 동기화** — 위키의 `~/hypomnema/extensions/{agents,commands,hooks,skills}/`에 둔 것은 자동으로 `~/.claude/`에 복사됩니다. `--codex` 옵션을 주면 `hooks`와 `commands`는 `~/.codex/`에도 추가로 복사됩니다 (agents·skills은 Claude 전용이라 Codex 쪽에서는 의도적으로 건너뜁니다 — ADR 0024).
> - **현재 디렉토리에 맞춰 프로젝트 자동 생성** — `cd`로 git 저장소(`package.json`, `Cargo.toml` 등 프로젝트 마커가 있는)에 들어갔는데 해당하는 위키 프로젝트가 없으면, 자동으로 만들지 물어봅니다 (ADR 0023).
> - **세션 종료 자동 정리 + `/clear` 복구** — 비자명한 세션이 끝날 때 "세션 종료 메모를 짧게 남기시겠습니까?"가 자동으로 뜨고, 정리 안 하고 `/clear` 친 경우도 다음 세션 시작 시 깔끔하게 이어집니다 (ADR 0022).
>
> 스키마(SCHEMA)는 2.0으로 올라갔습니다 — `feedback` 페이지 타입에 9개의 필수 frontmatter 필드가 추가됐습니다. `hypomnema upgrade --apply`를 돌리면 `MIGRATION-v2.0.md`가 위키 루트에 생기고, 거기에 단계별 보강 체크리스트가 들어있습니다. 사용자의 `SCHEMA.md`는 upgrade가 **절대 덮어쓰지 않습니다** (Option C라 부르는 정책 — 안내만 하고 diff는 사용자가 직접 적용). SCHEMA가 2.0으로 bump — `feedback` 페이지 9 hard-required 필드 강제; `hypomnema upgrade --apply` 시 `MIGRATION-v2.0.md`가 자동 작성됩니다 (Option C: 사용자 `SCHEMA.md`는 byte-equal로 보존).
