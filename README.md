...
>
> One case never parks: when the only difference is that TABLE ROWS were added inside a table that was already there. Every other line has to be there already, byte for byte and in the same order, blank lines included, and every added line has to be a row sitting in that table's body. That is the two-machines-editing-one-pointer-table case, and nothing can be lost by it. Anything else parks and waits for you.
