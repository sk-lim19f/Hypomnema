...
>
> One case never parks: when every line already on disk survives, unchanged and in the same order, inside the bytes your close is about to write. Nothing can be lost by that write, so it just happens. Anything else parks and waits for you.
