...
>
> When the change on disk only ADDED lines and kept every line your close was about to preserve, nothing was at risk and the close proceeds without parking anything. For the rest, `hypomnema proposal accept-base --session-id <id> <target>` says "keep what is on disk as my starting point", so the next close writes your page without parking it again. It needs the same approval an apply needs: a proposal parked on that exact target, a `hypomnema proposal challenge` covering it, and the approval line typed in the conversation. If the file changes again after that, it refuses: what was approved is the diff someone read, not whatever lands there later.
