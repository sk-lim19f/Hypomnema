...
>
> One case never parks: when the only difference is that TABLE ROWS were added. Every other line has to be there already, byte for byte and in the same order, and every added line has to be a row. That is the two-machines-editing-one-pointer-table case, and nothing can be lost by it. Anything else parks and waits for you.
