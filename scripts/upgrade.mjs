...
// hooks.json/shared.json consumer reads through — smoke-plugin, init, doctor,
// and uninstall all call the same function, so a command shape one of them
// would accept and another would silently misread cannot exist anymore. It also
// existence-checks every named file against THIS package's own hooks/ before
// returning ok:true, so a corrupt or incomplete package (a hook file dropped
// from `package.json`'s `files` allowlist) fails here, at startup, instead of
// upgrade reporting some files "src-missing" while still applying the rest.
