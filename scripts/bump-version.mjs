...
console.log(`     The READMEs need no edit: they describe current behavior, not release history.`);
console.log(`  3. Local pre-checks, the full gate set CI + prepublishOnly run:`);
console.log(`       npm test && npm run lint && npm run check:versions \\`);
console.log(`         && npm run check:bilingual \\`);
console.log(`         && npm run smoke:plugin && npm run smoke-pack && npm run check:tracker-ids`);
console.log(`  4. git add package.json package-lock.json .claude-plugin/ \\`);
console.log(`            templates/hypo-config.md CHANGELOG.md`);
console.log(`     git commit -m "chore(release): v${next}"`);
console.log(`     Then open the release PR, pass review + green CI, squash-merge to main.`);
console.log(`  5. Tag the MERGE COMMIT with an ANNOTATED tag (CI rejects lightweight tags):`);
console.log(`       git tag -a v${next} -m "<English body>\\n\\n---\\n\\n<한글 요약>"`);
console.log(`     See docs/CONTRIBUTING.md "Cutting a release" for the full template.`);
console.log(`  6. Rehearse against the tag (the same checks CI runs):`);
console.log(`       node scripts/check-bilingual.mjs --tag v${next}`);
console.log(`       node scripts/check-versions.mjs --tag v${next}`);
console.log(`       node scripts/check-tracker-ids.mjs --tag v${next}`);
console.log(`       npm publish --dry-run --access public   # packs, no registry PUT`);
console.log(`  7. git push origin <branch>   # then push the tag ALONE to trigger release:`);
