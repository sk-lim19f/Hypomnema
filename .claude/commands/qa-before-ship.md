...
5. **Spawn via /teams** — never direct CLI. Batch rows by surface into worker prompts. Each prompt MUST start with `export HOME=… CODEX_HOME=… HYPO_DIR=…` pointing into `$QA_SANDBOX`.
   ```
   /teams 3:codex "export HOME=/tmp/…/claude-home CODEX_HOME=/tmp/…/codex-home HYPO_DIR=/tmp/…/vault; scenarios 1–7 from <matrix-file>; for each: run in sandbox, capture output, verify against 'expected'. PASS/FAIL + evidence per row."
   ```
